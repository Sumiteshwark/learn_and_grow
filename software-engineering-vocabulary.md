...
### Provision (प्रोविज़न) / Provisioning (प्रोविज़निंग)
...
### Orchestrate (ऑर्केस्ट्रेट) / Orchestration (ऑर्केस्ट्रेशन)
...
### Spin up (स्पिन अप) / Tear down (टियर डाउन)
...
### Normalize (नॉर्मलाइज़) / Normalization (नॉर्मलाइज़ेशन)
...
### Consolidate (कंसोलिडेट) / Consolidation (कंसोलिडेशन)
...
### Reconcile (रिकंसाइल) / Reconciliation (रिकंसिलिएशन)
...
### Aggregate (एग्रीगेट) / Aggregation (एग्रीगेशन)
...
### Hydrate (हाइड्रेट) / Dehydrate (डीहाइड्रेट)
...
### Throttle (थ्रॉटल) / Rate-limit (रेट-लिमिट)
...
### Eventual consistency (इवेंचुअल कंसिस्टेंसी)
...
### Resilience (रिज़िलियंस)
**Meaning:** The ability of a system to recover from failures and continue operating under stress.
- *Real-world example:* A resilient microservice uses a circuit breaker so that if a dependency goes down, it fails fast rather than hanging and bringing down the entire system.
- *When to use it:* When discussing architecture design that anticipates and handles failure gracefully.
...
### Percentile latency (पर्सेटाइल लेटेंसी) (p95 / p99)
...
### Scalability (स्केलेबिलिटी) / Scale (स्केल)
...
### Cache hit (कैश हिट) / Cache miss (कैश मिस)
...
### Uptime (अपटाइम) / Downtime (डाउनटाइम)
...
### Deprecate (डेप्रिकेट) / Deprecated (डेप्रिकेटेड)
...
### Rollback (रोलबैक) / Rollout (रोलआउट)
...
### Single source of truth (सिंगल सोर्स ऑफ़ ट्रुथ)
...
### Technical debt (टेक्निकल डेट) (tech debt)
...
### Cohesion (कोहीज़न) / Coupling (कप्लिंग)
